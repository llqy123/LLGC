...
from utils import load_disease_data, fully_gen_rand_split
...
parser.add_argument('--dataset', type=str, default="disease",
                    help='Dataset to use.')
parser.add_argument('--lr', type=float, default=1.9,
                    help='Initial learning rate.')
parser.add_argument('--weight_decay', type=float, default=8.2e-09,
...
parser.add_argument('--K', type=int, default=24, help='number of propagation steps.')
parser.add_argument('--drop_out', type=float, default=0.0, help='Hyperbolic linear drop_out')
parser.add_argument('--use_bias', type=int, default=1, help='Hyperbolic linear bias(1 or 0)')
...
def main(args):
    # load data
    data_path = r'data/disease'
    adj, features, labels = load_disease_data(args.dataset, data_path, args.normalization, args.cuda)
    split_path = f'data/data_splits/{args.dataset}_idx.pt'
    try:
        idx_train, idx_val, idx_test = torch.load(split_path)
    except:
        idx_train, idx_val, idx_test = fully_gen_rand_split(len(labels), val_ratio=0.1, test_ratio=0.6)
        torch.save([idx_train, idx_val, idx_test], split_path)
    sgconv = PageRankAgg(args.K, alpha=args.alpha).to("cuda")
